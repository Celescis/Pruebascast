...
public class VideoProvider {
    ...
    */
    public static List<MediaInfo> buildMedia() throws IOException {
        mediaList = new ArrayList<>();

        // URL del stream principal de video
        String videoUrl = "https://cdn.bitmovin.com/content/assets/sintel/hls/playlist.m3u8";

        // Descargar y analizar el archivo .m3u8 para construir las pistas de audio
        List<MediaTrack> audioTracks = getAudioTracksFromM3U8(videoUrl);

        // Construir la información del medio
        MediaInfo mediaInfo = buildMediaInfo(
                "Titulo del Video", // Título
                "Estudio", // Estudio
                "Descripción del video", // Subtítulo o descripción
                0, // Duración en segundos (0 para streams en vivo)
                videoUrl, // URL del vídeo
                "application/x-mpegURL", // MimeType para HLS
                "http://url-to-thumbnail.jpg", // URL de la miniatura (cambiar según sea necesario)
                "http://url-to-bigimage.jpg", // URL de la imagen grande (cambiar según sea necesario)
                audioTracks // Pistas de audio generadas automáticamente
        );
    ...
    private static List<MediaTrack> getAudioTracksFromM3U8(String m3u8Url) throws IOException {
        List<MediaTrack> tracks = new ArrayList<>();
        URL url = new URL(m3u8Url);
        try (InputStream is = url.openStream();
             BufferedReader reader = new BufferedReader(new InputStreamReader(is))) {
            String line;
            int trackId = 1; // Iniciar contador de ID de pista
            while ((line = reader.readLine()) != null) {
                if (line.startsWith("#EXT-X-MEDIA:")) {
                    MediaTrack track = parseMediaTrack(line, trackId++);
                    if (track != null) {
                        tracks.add(track);
                    }
                }
    ...
    private static MediaTrack parseMediaTrack(String mediaLine, int trackId) {
        Pattern pattern = Pattern.compile("TYPE=AUDIO.*LANGUAGE=\"(.+?)\".*NAME=\"(.+?)\".*URI=\"(.+?)\"");
        Matcher matcher = pattern.matcher(mediaLine);
        if (matcher.find()) {
            String language = matcher.group(1);
            String name = matcher.group(2);
            String uri = matcher.group(3);
            return new MediaTrack.Builder(trackId, MediaTrack.TYPE_AUDIO)
